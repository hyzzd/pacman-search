...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # ghost captured by Pacman
            allPossible[self.getJailPosition()] = 1.0 
        else:
            for p in self.legalPositions: # only possible ghost positions
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        for oldPos in self.legalPositions: # obtain the distribution over new positions for the ghost
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items(): # loop over key, value pairs in newPosDist
                allPossible[newPos] += prob * self.beliefs[oldPos]

        self.beliefs = allPossible # update belief
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [] # initialize list
        numPositions = len(self.legalPositions) # get number of legal positions
        for pos in self.legalPositions: # Evenly distribute particles across positions
            for i in range(self.numParticles / numPositions): # repeat uniformly
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter() # initialize
        weightDistribution = self.getBeliefDistribution() # given belief distribution
        if noisyDistance == None: # ghost captured by Pacman case
            self.particles = [self.getJailPosition()] * self.numParticles
        else:
            for p in self.legalPositions:
                partDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] += emissionModel[partDistance] * weightDistribution[p]
            if not any(allPossible.values()): # all particles receive 0 weight case
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(allPossible) for i in range(self.numParticles)] # sample from belief distribution
        #util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        tmpList = []
        for oldPos in self.particles: # as before
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            tmpList.append(util.sample(newPosDist)) # sample from distribution
        self.particles = tmpList
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            dist[particle] += 1 # convert particles into distribution
        dist.normalize() # normalize distribution
        return dist
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [] # initialize list
        count = 0
        numParticles = self.numParticles # get number of particles
        permutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts)) # use given package
        random.shuffle(permutations) # shuffle the list to ensure even placement of particles
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            prob = 1.0
            for i in range(self.numGhosts): # loop over ghosts
                if noisyDistances[i] == None: # ghost captured case
