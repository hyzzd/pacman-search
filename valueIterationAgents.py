...
class ValueIterationAgent(ValueEstimationAgent):
          ...
          for state in self.mdp.getStates():
            ...
            for action in self.mdp.getPossibleActions(state):
              for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
                tmpValues[action] += prob * (mdp.getReward(state, action, nextState) + self.discount * valuesCopy[nextState])
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        Q_value = 0 # Q-value
        for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action): # Q = SIGMA P*(r + y*V)
    ...
    def computeActionFromValues(self, state):
        ...
        for action in self.mdp.getPossibleActions(state):
          ...
          if value <= temp: # find action of max value
            value = temp
            result = action
